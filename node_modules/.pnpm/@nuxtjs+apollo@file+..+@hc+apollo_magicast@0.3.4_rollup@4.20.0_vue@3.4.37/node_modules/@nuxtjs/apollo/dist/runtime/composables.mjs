...
export async function useAsyncMutation(...args) {
  const { variables, mutate } = prepMutation(...args);
  const result = await mutate();
  return { result, variables };
}
const prepMutation = (...args) => {
  const { clients } = useApollo();
  const mutation = args?.[0]?.mutation || args?.[0];
  const cache = args?.[0]?.cache ?? true;
  const variables = args?.[0]?.variables || typeof args?.[1] !== "string" && args?.[1] || void 0;
  ...
  const key = args?.[0]?.key || hash({ mutation: print(mutation), variables, clientId });
  const mutate = () => clients[clientId]?.mutate({ mutation, variables }).then((r) => r.data);
  return { key, mutation, clientId, variables, mutate };
};
